...
def train_and_test(model):
    ...
    for epoch in tqdm(range(parameters.epochs)):
        ...
        df_true=pd.DataFrame(list(dict_true.items()),columns=['point','counts'])
        df_predict=pd.DataFrame(list(dict_predict.items()),columns=['point','counts'])
        sns.barplot(x='point',y='counts',data=df_true)
        plt.title("true")
        plt.show()
        sns.barplot(x='point',y='counts',data=df_predict)
        plt.title("predict")
        plt.show()
        #print(dict_true)
        #print(dict_predict)
